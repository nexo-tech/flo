...
let random_hex_string len =
  ...
  for i = 0 to Bytes.length bytes - 1 do
    Printf.bprintf buf "%02x" (Char.code (Bytes.get bytes i))
  done;
  Buffer.contents buf

(* Helper: Check if string is all zeros *)
let is_all_zeros s =
  String.for_all (fun c -> c = '0') s
...
let parse_traceparent header =
  ...
  | [version; trace_id; span_id; flags] ->
      ...
      (* Validate trace_id *)
        ...
        Error (Printf.sprintf "Invalid trace_id: %s (must be 32 hex chars, not all zeros)" trace_id)
      (* Validate span_id *)
      else if not (is_valid_span_id span_id) then
        Error (Printf.sprintf "Invalid span_id: %s (must be 16 hex chars, not all zeros)" span_id)
      (* Validate flags *)
  ...
  | _ ->
      Error "Invalid traceparent format (expected: 00-{trace_id}-{span_id}-{flags})"
